...
[betacoef,~,~,CovB,MSE,~] = lsr(x,y,modelfun);

%computed values using matlab
A = [x(:).^2 x(:) ones(size(x(:)))];
L = y(:);
[matlab_betacoef, ~, matlab_MSE, matlab_CovB] = lscov(A,L); %same results
...
checkVals(MSE,matlab_MSE,THRESH);
...
W = [1 10 100 5 1];
modelfun = @(b,x) b(1)*x.^2 + b(2)*x + b(3); 
[betacoef,~,~,CovB,MSE,~] = lsr(x,y,modelfun,'weights',W);

% computed values using matlab
A = [x(:).^2 x(:) ones(size(x(:)))];
L = y(:);
[matlab_betacoef, ~, matlab_MSE, matlab_CovB] = lscov(A,L,W); %same results
...
Sbb = diag([0.1 0.1 1]);
modelfun = @(b,x) b(1)*x.^2 + b(2)*x + b(3); 
[betacoef,~,~,CovB,MSE,~] = lsr(x,y,modelfun,betaest,'weights',Syy,'betaCoef0Cov',Sbb);
...
modelfun = @(b,x) b(1)*exp(b(2)*x);
beta0 = [1 1]';
[betacoef,R,~,CovB,MSE,~] = lsr(x,y,modelfun,beta0,'Weights',W);

%computed values using matlab
[betacoef2,R2,~,CovB2,MSE2,~] = nlinfit(x,y,modelfun,beta0,'weights',W);
%check to make sure function is working correctly
checkVals(betacoef,betacoef2,THRESHNLIN);
checkVals(R,R2,THRESHNLIN*10); %loose check thresh, lsr is actually better
checkVals(CovB,CovB2,THRESHNLIN*10);
checkVals(MSE,MSE2,THRESHNLIN*10);
...
[betacoef,R,~,CovB,MSE,~] = lsr(x,y,modelfun,'RobustWgtFun','bisquare');
[betacoef2, stats] = robustfit(x,y);

checkVals(betacoef,betacoef2,THRESHNLIN);
checkVals(R,-stats.resid,THRESHNLIN*10); %loose check thresh, lsr is actually better
checkVals(CovB./MSE,stats.covb/stats.s^2,THRESHNLIN*10); %check unweighted Covariance
% checkVals(MSE,MSE2,THRESHNLIN*10); 
%Unsure how to compute MSE with matlab
...
beta0 = [2;2;2];
[betacoef,R,~,CovB,MSE,~] = lsr(x,y,modelfun,beta0,'RobustWgtFun','bisquare');

opts = statset('nlinfit');
opts.RobustWgtFun = 'bisquare';
[betacoef2,R2,~,CovB2,MSE2,~] = nlinfit(x,y,modelfun,beta0,opts);

checkVals(betacoef,betacoef2,THRESHNLIN);
checkVals(R,R2,THRESHNLIN*10); %loose check thresh, lsr is actually better
checkVals(CovB./MSE,CovB2./MSE2,THRESHNLIN*10); %check unweighted Covariance
% checkVals(MSE,MSE2,THRESHNLIN*10); 
%Unsure how to compute MSE with matlab
...
[betacoef,R,~,CovB,MSE,~] = lsr([x y],zeros(size(y)),modelfun,beta0,'Weights',S,'type','tls');
